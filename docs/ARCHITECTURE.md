...
| 91 | May 2026 | UI tests excluded from Xcode Cloud PR CI gate | Persistent Apple infrastructure instability (AX loaded notification timeout) causes PRLiftsUITests to fail non-deterministically on Xcode Cloud runners, breaking PRs for reasons unrelated to code changes. Compensating controls: (1) pre-commit hook runs the full test suite including PRLiftsUITests on the local iPhone SE simulator before every commit — no commit reaches the PR without UI tests passing locally; (2) a weekly scheduled Xcode Cloud build runs the full suite (PRLiftsCITests + PRLiftsUITests) on main every Monday at 09:00 UTC, providing a weekly green signal. Unit tests (PRLiftsTests) remain required on every PR via the PRLiftsCITests test plan. `git commit --no-verify` is never permitted — bypassing the hook removes the only hard gate on UI tests. |
