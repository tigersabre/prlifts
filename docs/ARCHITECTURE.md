...
| 87 | April 2026 | PR recalculation is automatic on WorkoutSet edit | Any edit to a WorkoutSet triggers the PR detection engine to re-evaluate all PersonalRecords for that exercise and user. PRs are updated or removed if they no longer qualify; new PRs created if the edit reveals a previously undetected best. Keeps PR data consistent with actual logged data at all times. |
