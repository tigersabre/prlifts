...
| 77 | April 2026 | Scrum sprint model adopted — one week sprints | Fixed cadence, defined scope, potentially shippable increment each Friday. Kanban rejected — too freeform for a product with a defined roadmap. |
| 78 | April 2026 | Four development teams defined | Backend, iOS, Infrastructure, UX/Design. Each story owned by exactly one team. Cross-cutting concerns (QA, Security, DevOps, Legal) embedded as labels and checklists, not separate teams. |
| 79 | April 2026 | Design-one-sprint-ahead rule | UX/Design produces screen specs in sprint N. iOS builds against them in sprint N+1. iOS never starts a UI story without an approved design spec. |
| 80 | April 2026 | QA, Security, DevOps, Legal embedded not siloed | These are not separate teams — they are labels, checklists, and blocking conditions on individual stories. Every story that touches user data runs the security checklist. Every story has a Definition of Done checklist. |
| 81 | April 2026 | GitHub Projects sprint board as project management tool | Scrum board with iterations, five columns (Backlog → Sprint Backlog → In Progress → In Review → Done), four views (Current Sprint, Backlog, By Team, By Version, Blocked). |
| 82 | April 2026 | Claude Code treated as a team member within sprint structure | Works one story at a time, reads CLAUDE.md at session start, follows Definition of Done, opens PRs for human review. Does not make product or visual design decisions independently. |
| 83 | April 2026 | Claude Design adopted as UX/Design tool | Claude Design (claude.ai/design, powered by Opus 4.7) replaces Figma for all wireframes, mockups, and interactive prototypes. Reads repo directly (DESIGN_TOKENS.md, SCREEN_INVENTORY.md, USER_FLOWS.md). Handoff bundle passed directly to Claude Code — no manual translation. Closed loop: Claude Design → handoff bundle → Claude Code → production SwiftUI. |
| 84 | April 2026 | PyJWT chosen over python-jose for Supabase JWT validation | PyJWT is actively maintained, has no known CVEs, and requires no extras for HS256 (the algorithm Supabase uses). python-jose has had security vulnerabilities in past releases and is less actively maintained. Supabase JWTs are HS256-signed with SUPABASE_JWT_SECRET; audience claim validated as "authenticated". |
| 85 | April 2026 | Exercise videos surfaced via Demo button on ActiveWorkoutScreen and full ExerciseDetailScreen | EDB video URLs stored on exercise record and played via AVPlayer on iOS. Demo button appears inline during active workout for quick reference; ExerciseDetailScreen provides full-screen detail view with video, muscle groups, and instructions. |
| 86 | April 2026 | Full CRUD on all workout data — sets, exercises, and workouts editable and deletable during and after workout | Users must be able to correct logging mistakes post-completion. Edit and delete available on WorkoutSets, WorkoutExercises, and Workouts in both ActiveWorkoutScreen and WorkoutDetailScreen. |
| 87 | April 2026 | PR recalculation scans ALL historical sets for the exercise and user on edit | Any edit to a WorkoutSet triggers the PR detection engine to scan every WorkoutSet for that exercise across every workout for that user — not just the edited workout. PRs are updated or removed if they no longer qualify; new PRs are created if the edit reveals a previously undetected best. The idx_workout_set_exercise_user index covers this cross-workout scan. |
| 88 | April 2026 | PR recalculation on set edit is synchronous in V1; async in V2 | V1 complexity does not justify the operational overhead of a background worker. Synchronous recalculation completes in < 200ms on expected V1 data volumes. At scale (V2) this migrates to Celery + Redis to avoid blocking the set-edit API response. |
| 89 | April 2026 | ExerciseDetailScreen CTA is contextual on workout state | "Add to workout" CTA shown when a workout is in progress (→ ActiveWorkoutScreen). "Start workout with this exercise" CTA shown when no workout is active (→ ActiveWorkoutScreen, new workout created with this exercise pre-loaded). No workout state = no ambiguity about the user's intent. |
| 90 | April 2026 | Schema consistency enforced via [iOS]/[BE] annotations | (1) [iOS]/[BE] column annotations in SCHEMA.md as single source of truth for both PostgreSQL and SwiftData; (2) paired update requirement in Definition of Done — any story touching an [iOS] column must update the Alembic migration, SwiftData model, and SCHEMA.md in the same PR; (3) schema consistency CI test in Sprint 2 parses annotations and validates SchemaMapping.swift committed to PRLiftsCore; (4) BiometricConsent is permanently [BE] — creating a SwiftData model for it is a blocking security and legal violation, not a style preference. PostgreSQL snake_case maps deterministically to SwiftData camelCase. |
| 94 | May 2026 | Cursor-based (keyset) pagination. Cursor = (created_at DESC, id DESC) composite, base64-encoded. Applied to all list endpoints. | Offset pagination produces duplicates and gaps when new records are inserted during paging. |
| 95 | May 2026 | Account hard delete: synchronous explicit cascade, Fal.ai images queued for manual cleanup, biometric_consent retained 1 year (BIPA). audit_log and image_deletion_queue use plain UUID with no FK so records survive user deletion. SupabaseAdminClient is an injectable Protocol so tests stub auth.users deletion without a real Supabase connection. Rate limit: 1 req/user/hour via Redis (separate from the 60-second general middleware window). | Hard delete chosen over soft delete: GDPR/CCPA require erasure; soft delete retains PII indefinitely. Synchronous (not async job): 30-second window is achievable and avoids a job queue dependency for a rare, user-initiated operation. |
| 91 | May 2026 | UI tests excluded from Xcode Cloud PR CI gate; pre-commit hook runs unit tests only (PRLiftsCITests) | Persistent Apple infrastructure instability causes PRLiftsUITests to fail non-deterministically on Xcode Cloud runners. The pre-commit hook originally ran the full suite (PRLiftsWeeklyTests) but the UI test suite grew past 720s by Sprint 3, exceeding the 600s xcodebuild hard timeout and causing the hook to report failure even when tests passed. Resolution: pre-commit hook uses PRLiftsCITests (unit tests only, ~45s) for every commit. Compensating control: a weekly scheduled Xcode Cloud build runs the full suite (PRLiftsCITests + PRLiftsUITests) on main every Monday at 09:00 UTC, providing a weekly green signal on UI tests. Unit tests (PRLiftsTests) remain required on every commit and every PR. `git commit --no-verify` is never permitted. |
| 92 | May 2026 | HomeScreen consistency metric is weekly workout count vs personal weekly average — not a daily streak | Format: "3 of 4 workouts this week." Weightlifting requires planned rest days; daily streaks punish good training behavior and generate anxiety when broken for legitimate reasons (injury, recovery, travel). Weekly metrics reward consistency without streak-break shame. The 7-day progress bar fills across the week and resets each Monday. "Best Week" stat shows the user's personal record for workouts in a single week — tying the consistency mechanic back to the PR theme. No streak count is shown anywhere in V1. |
| 93 | May 2026 | Sync conflict resolution is last-write-wins with duplicate prevention | Workout logging is append-heavy, not edit-heavy — the vast majority of writes are new sets, not edits to existing ones. True conflicts are rare; the complexity of flagging conflicts to users is not justified at V1 scale. Duplicate prevention rule: if the same WorkoutSet (same workout_exercise_id, same weight, same reps, same weight_modifier) is uploaded within 30 seconds of an existing identical set, the backend deduplicates rather than creating two records. This handles the most common real-world conflict scenario — user logging a set twice after connectivity loss — without requiring user intervention. |
| 96 | May 2026 | Direct asyncpg pool for all repository reads and writes; service-role credentials bypass RLS; application-layer `WHERE user_id = $1` provides isolation | SQLAlchemy ORM adds complexity for no benefit given the already-defined Protocols. Direct asyncpg gives full control over SQL, enum casts, and JSONB handling. Service-role connection bypasses Supabase RLS (safe because every query filters `WHERE user_id = $1`). Pool conditionally created on startup; skipped when `DATABASE_URL` is absent so unit tests run without a database. Asyncpg concrete classes are excluded from the 90% coverage gate (`app/repositories/asyncpg_*.py`) — they are infrastructure not business logic, and their correctness is validated by integration tests against a real PostgreSQL instance. |
